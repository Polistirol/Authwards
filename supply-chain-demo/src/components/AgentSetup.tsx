  ...
  async function handleCreate(): Promise<void> {
    ...
    try {
      const res = await createAgent({
        ...
        description:
          "Pagamento automatico al fornitore quando la spedizione risulta consegnata (workflow esterno n8n).",
          ...
          <div className="border-b border-slate-600 p-6">
            <div className="flex items-start justify-between gap-3">
              <h2 id="agent-setup-title" className="text-xl font-semibold text-white">
                {success
                  ? "Identità creata"
                  : `Configura Agente per ${shipment.product}`}
              </h2>
              <button
                type="button"
                onClick={handleClose}
            ...
            {success ? (
              <div className="space-y-4">
                <p className="text-sm leading-relaxed text-slate-300">
                  Collega questo agente al tuo workflow n8n o al tuo bot per automatizzare il pagamento.
                </p>
                <div className="rounded-xl border border-slate-600 bg-[#131a2a] p-4">
                  <p className="text-xs uppercase text-slate-500">DID agente</p>
                  <code className="mt-1 block break-all text-sm text-amber-400">{success.agentDid}</code>
                  <p className="mt-3 text-xs uppercase text-slate-500">Stato</p>
                  <p className="mt-1 text-sm font-medium text-amber-200">In attesa di attivazione</p>
                </div>
                <button
                  type="button"
                ...
                >
                  Vedi Snippet per collegare l&apos;agente
                </button>
              </div>
            ) : (
              <>
                <p className="text-sm leading-relaxed text-slate-300">
                  Crea un&apos;identità agente sulla piattaforma IOTA Auth. Poi collega un workflow esterno
                  (n8n, bot, script) per automatizzare il pagamento.
                </p>
                <div className="mt-5 space-y-2 rounded-xl border border-slate-600 bg-[#131a2a] p-4 text-sm text-slate-300">
                  ...
                  <p>
                    <span className="text-slate-500">Azione:</span> rilascio pagamento al delivery
                  </p>
                  <p>
                ...
                <fieldset className="mt-6">
                  <legend className="text-sm font-medium text-white">Profilo permessi</legend>
                  <div className="mt-3 space-y-3">
                    ...
                    <label className="flex cursor-pointer gap-3 rounded-xl border border-slate-600 bg-[#131a2a] p-4 has-[:checked]:border-amber-500/60">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-400">nessun limite pratico (full_access)</p>
                      </div>
                    </label>
                  </div>
                </fieldset>
                ...
                >
                  {busy ? "Creazione…" : "Crea Identità Agente"}
                </button>
              </>
            )}
          </div>
